...
//  ManicEmu
//
//  Created by Aushuang Lee on 2025/2/4.
...
- (void)updateGameSettings
{
    ...
    NSString *iniPath = [NSSearchPathForDirectoriesInDomains(
        NSLibraryDirectory, NSUserDomainMask, YES
    )[0] stringByAppendingPathComponent:@"System.bundle/vba-over.ini"];
...
- (BOOL)addCheatCode:(NSString *)cheatCode type:(NSString *)type
{
    ...
    for (NSString *code in codes)
    {
        ...
        if ([type isEqualToString:@"ActionReplay"] || [type isEqualToString:@"GameShark"])
        {
        ...
        else if ([type isEqualToString:@"CodeBreaker"])
        {
            if (code.length != 13)
            {
                return NO;

...
extension UIView {
    ...
    static func makeToast(message: String, isRemovable: Bool = true, identifier: String? = nil, hideCompletion: (()->Void)? = nil) {
        ...
        if !isRemovable, let identifier = identifier {
            ...
            Toast(.message(message).duration(3).identifier(identifier)) { toast in
                setupToast(toast: toast)
            }
            
        } else {
            
            Toast(.message(message).duration(3)) { toast in
                setupToast(toast: toast)
            }
        }
    }

...
extension UIDevice {
    ...
    static var isPhone: Bool {
        if Device.isPhone() || Device.isPod() {
            ...
            if Constants.Size.WindowSize.minDimension < 744 { 
                return true
            }
        }
        return false
    ...
    static var deviceInfo: String {
        let memberType = PurchaseManager.isAnnualMember ? 1 : (PurchaseManager.isMonthlyMember ? 2 : (PurchaseManager.isForeverMember ? 3 : 0))
        return "\(StableID.id)\nDevice:\(Device.version())\nVersion:\(UIDevice.current.systemVersion)\niCloudEnable:\(UserDefaults.standard.bool(forKey: "iCloudSyncEnable"))\nMemberType:\(memberType)"
    }
}

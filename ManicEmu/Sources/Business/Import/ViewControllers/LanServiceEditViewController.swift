...
//  ManicEmu
//
//  Created by Aushuang Lee on 2025/2/27.
        ...
        let view = HowToButton(title: R.string.localizable.landServiceEditConnect()) { [weak self] in
            ...
            if self.service.type == .samba {
                ...
                if let host = service.host, let scheme = service.scheme {
                    ...
                    let webDAV = WebDAV(baseURL: host, port: service.port ?? (scheme == "http" ? 80 : 443), username: service.user, password: service.password, path: service.path)
                    Task {
                        do {

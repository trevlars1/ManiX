...
class SettingsListFooterCollectionReusableView: UICollectionReusableView {
    override init(frame: CGRect) {
        ...
        userInfoContainer.addTapGesture { gesture in
            UIPasteboard.general.string = StableID.id
            UIView.makeToast(message: R.string.localizable.idCopy())
        }
        ...
        let userID = UILabel()
        userID.textColor = Constants.Color.LabelTertiary
        userID.font = Constants.Font.caption(size: .l)
        userID.text = "ID \(StableID.id)"
        userInfoContainer.addSubview(userID)
        userID.snp.makeConstraints { make in
            make.leading.centerY.equalToSuperview()
        }
        let copyIcon = UIImageView(image: R.image.customDocumentOnClipboardFill()?.applySymbolConfig(font: Constants.Font.caption(size: .l), color: Constants.Color.LabelTertiary))
        userInfoContainer.addSubview(copyIcon)
        copyIcon.snp.makeConstraints { make in
            make.trailing.top.bottom.equalToSuperview()
            make.leading.equalTo(userID.snp.trailing).offset(Constants.Size.ContentSpaceUltraTiny)
        }
    }

...
//    }
    ...
    private func updateGames() {
        ...
        gamesUpdateToken = games.observe(keyPaths: [\Game.aliasName, \Game.gameCover, \Game.latestPlayDate, \Game.latestPlayDuration, \Game.totalPlayDuration]) { [weak self] changes in
            guard let self = self else { return }
            if case .update(_, _, let insertions, let modifications) = changes {

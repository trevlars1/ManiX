...
class GamesListBlankSlateView: BaseView {
    ...
    override init(frame: CGRect) {
        ...
        let guideImageView: UIImageView = {
            ...
            gradientLayer.frame = CGRect(origin: .zero, size: Constants.Size.IconSizeTiny)
            gradientLayer.colors = Constants.Color.Gradient.reversed().map({ $0.cgColor })
            gradientLayer.startPoint = CGPoint(x: 0.0, y: 0.5)
            gradientLayer.endPoint = CGPoint(x: 1.0, y: 0.5)
            view.layer.addSublayer(gradientLayer)
            let maskLayer = CALayer()
            maskLayer.contents = UIImage(symbol: .trayAndArrowDown).withRenderingMode(.alwaysTemplate).cgImage
        ...
        channelLinkLabel.attributedText = matt.applying(attributes: [.foregroundColor: Constants.Color.Red], toOccurrencesOf: channelName)

...
class GameSettingView: BaseView {
    ...
    static func estimatedHeight(for contentWidth: CGFloat, hasGrabber: Bool = true) -> CGFloat {
        let line = 4
        let itemHeight = (contentWidth - totalHorizontalSpacing) / CGFloat(line) 
    ...
    init(game: Game, isEditingMode: Bool = false) {
        self.game = game
    ...
    private func updateSettings() {
        ...
        if game.gameType == ._3ds && !isEditingMode {
            let disableFor3DS = GameSetting.disableFor3DS
            gameSettings = gameSettings.filter({ !disableFor3DS.contains([$0.type]) })
        }
...
extension GameSettingView: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.setData(item: item, editable: isEditingMode, isPlus: indexPath.section != 0)
        if isEditingMode {
            cell.editButton.addTapGesture { [weak self] gesture in
                ...
                if indexPath.section == 0 {
                    ...
                    self.collectionView.performBatchUpdates {
                        ...
                        self.collectionView.moveItem(at: indexPath, to: IndexPath(row: toIndex, section: 0)) 
                    } completion: { _ in
                        if self.displayGamesFunctionCount > Constants.Numbers.GameFunctionButtonCount {
...
extension GameSettingView: UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        case .haptic:
            ...
            return
//        case .controllerType:
//            item.controllerType = item.controllerType.next
//            updateCellAndCallBack(item: item, indexPath: indexPath)
//            return
        case .orientation:
    ...
    func collectionView(_ collectionView: UICollectionView, contextMenuConfigurationForItemsAt indexPaths: [IndexPath], point: CGPoint) -> UIContextMenuConfiguration? {
        guard let indexPath = indexPaths.first, !isEditingMode else { return nil }
        var item = gameSettings[indexPath.row]
